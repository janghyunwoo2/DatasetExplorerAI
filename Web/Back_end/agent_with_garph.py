...
examples =[
    {
        "input": "국내 미세먼지 농도 데이터를 구하고 싶어.",
        "output": "국내 환경 데이터는 '공공데이터포털(data.go.kr)'에서 한국환경공단이 제공하는 '대기오염정보'를 확인하는 것이 가장 정확합니다. 미세먼지, 초미세먼지 농도를 시간대별로 확인하실 수 있습니다."
    },
    {
        "input": "국내 지역별 카페 개수를 알고 싶어.",
        "output": "'공공데이터포털(data.go.kr)'에서 '커피'또는'카페'등의 키워드로 검색하여 제공 받을 수 있습니다."
    },
    {
        "input": "글로벌 IT 기업들의 매출 성장률 통계가 필요해.",
        "output": "해외 데이터의 경우, 일차적으로 '국가통계포털(KOSIS)'의 국제통계 섹션을 확인해 보시고, 구체적인 기업 데이터는 '구글 데이터셋(Google Dataset Search)'이나 '캐글(Kaggle)'을 통해 수집하시는 것을 권장합니다."
    }
]
...
final_prompt = ChatPromptTemplate.from_messages([
    ...
    사용자의 요청에 맞는 데이터셋을 추천하고, 출처를 모두 URL 형태로 알려주세요.
    필요하면 도구를 사용하세요.
